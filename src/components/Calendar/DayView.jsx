...
export default function DayView({
  ...
  // 시간 없는 일정 / 시간 있는 일정 분리
  const noTimeEvs  = dayEvs.filter(e => !e.start_time)
  const hasTimeEvs = dayEvs.filter(e => !!e.start_time)
        ...
        <div className="dv-left">
          ...
          {dayEvs.length === 0 ? (
            <div className="dv-empty">
              ...
              <button className="btn btn-outline-sm" style={{ marginTop: 10 }}
                onPointerUp={() => onAddEvent(currentDate)}>일정 추가</button>
            </div>
          ) : (
            <div className="dv-event-list">
              ...
              {noTimeEvs.length > 0 && (
                <>
                  ...
                  {noTimeEvs.map(ev => {
                    ...
                    return (
                      <div key={ev.id} className="dv-card" style={{ borderLeftColor: color }}
                        onPointerUp={() => onEventClick(ev)}>
                        <div className="dv-dot" style={{ backgroundColor: color }} />
                        <div className="dv-info">
                          <div className="dv-name">{ev.assignee}</div>
              ...
              {hasTimeEvs.length > 0 && (
                <>
                  <div className="dv-sub-label" style={{ marginTop: noTimeEvs.length > 0 ? 12 : 0 }}>
                    시간 지정 ({hasTimeEvs.length}건)
                  </div>
                  {hasTimeEvs.map(ev => {
                    const user  = users.find(u => u.id === ev.user_id)
                    const color = user?.color || ev.color || '#4F8EF7'
          ...
          <div className="dv-actions">
            ...
            {dayEvs.length > 0 && (
              <button className="btn btn-danger" onPointerUp={onDeleteDay}>
          ...
          {users.length > 0 && (
            <div className="dv-assign-block">
              ...
              <div className="dv-assign-chips">
                {users.map(user => (
                  <button key={user.id} className="dv-chip-btn"
                    style={{ backgroundColor: user.color || '#4F8EF7', color: '#fff' }}
                    onPointerUp={() => onQuickAssign(ds, user)}>
                    {user.name}
                  </button>
                ))}
              </div>
            ...
            <div className="dv-assign-block">
              ...
              <div className="dv-assign-chips">
                {groups.map(group => {
                  ...
                  return (
                    <button key={group.id} className="dv-chip-btn"
                      style={{ backgroundColor: group.color || '#6366F1', color: '#fff', display: 'flex', alignItems: 'center', gap: '5px' }}
